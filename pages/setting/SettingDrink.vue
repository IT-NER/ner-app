            ...
            <v-card-text>
              ...
              >
                ...
                <template v-slot:item.actions="{ item }">
                  ...
                  <v-btn color="error" small @click="deleteDrink(item)">
                ...
                <template v-slot:no-data>
                  <v-btn color="primary" @click="getDrink"> Reset </v-btn>
                </template>
              </v-data-table>
            </v-card-text>
          </v-card>
...
export default {
  data: () => ({
    ...
    headers: [
      ...
      { text: "ชื่อเครื่องดื่ม", value: "name" },
      {
        text: "Actions",
        value: "actions",
        align: "center",
  ...
  methods: {
    ...
    async deleteDrink(item) {
      ...
      if (delateConfirm) {
        let drink = await this.$axios.delete(
          "/api/drink/" + this.editedItem.id
        );
    ...
    async save(e) {
      ...
      if (saveConfirm) {
        ...
        if (this.editedIndex > -1) {
          drink = await this.updateDrink();
        } else {
          drink = await this.createDrink();
        }
    ...
    async updateDrink() {
      let updateDrink = await this.$axios
        .put("/api/drink/" + this.editedItem.id, {
          data: this.editedItem,
        })
        .then((res) => {
          // console.log("res", res.data);
