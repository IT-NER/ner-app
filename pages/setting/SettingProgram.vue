            ...
            <v-card-text>
              ...
              >
                ...
                <template v-slot:item.actions="{ item }">
                  ...
                  <v-btn color="error" small @click="deleteProgram(item)">
                ...
                <template v-slot:no-data>
                  <v-btn color="primary" @click="getProgram"> Reset </v-btn>
                </template>
              </v-data-table>
            </v-card-text>
          </v-card>
...
export default {
  ...
  methods: {
    async getProgram() {
      this.programs = await this.$axios
        .get("/api/program")
        .then((res) => {
    ...
    async deleteProgram(item) {
      ...
      if (delateConfirm) {
        let program = await this.$axios.delete(
          "/api/program/" + this.editedItem.id
        );
    ...
    async save(e) {
      ...
      if (saveConfirm) {
        ...
        if (this.editedIndex > -1) {
          program = await this.updateProgram();
        } else {
    ...
    async updateProgram() {
      let updateProgram = await this.$axios
        .put("/api/program/" + this.editedItem.id, {
          data: this.editedItem,
        })
        .then((res) => {
          // console.log("res", res.data);
