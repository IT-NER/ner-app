  ...
  <div>
    <v-card>
      ...
      <v-toolbar elevation="0">
        ...
        <v-tooltip top>
          <template v-slot:activator="{ on, attrs }">
            <v-btn v-bind="attrs" v-on="on" color="primary" @click="getRole">
              รีเฟรช
            </v-btn>
          </template>
...
export default {
  ...
  data() {
    return {
      ...
      headers: [
        ...
        { text: "สิทธิ์การใช้งาน", value: "name" },
  ...
  methods: {
    ...
    async create() {
      let createRole = await this.$axios
        .post("/api/role", {
          data: this.role,
        })
        .then((res) => {
    ...
    async update() {
      let updateRole = await this.$axios
    ...
    async editItem(item) {
      this.role = await Object.assign({}, item);
      this.dialog = true;
    },
