            ...
            <v-card-text>
              ...
              >
                ...
                <template v-slot:item.actions="{ item }">
                  ...
                  <v-btn color="error" small @click="deleteDepartment(item)">
                ...
                <template v-slot:no-data>
                  <v-btn color="primary" @click="getDepartment"> Reset </v-btn>
                </template>
              </v-data-table>
            </v-card-text>
          </v-card>
...
export default {
  ...
  methods: {
    ...
    async deleteDepartment(item) {
      ...
      if (delateConfirm) {
        let department = await this.$axios.delete(
          "/api/department/" + this.editedItem.id
        );
    ...
    async save(e) {
      ...
      if (saveConfirm) {
        ...
        if (this.editedIndex > -1) {
          department = await this.updateDepartment();
        } else {
    ...
    async updateDepartment() {
      let updateDepartment = await this.$axios
        .put("/api/department/" + this.editedItem.id, {
          data: this.editedItem,
        })
        .then((res) => {
          // console.log("res", res.data);
