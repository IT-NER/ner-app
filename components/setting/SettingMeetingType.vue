...
export default {
  ...
  methods: {
    ...
    async editItem(item) {
      this.editedIndex = this.meetingTypes.indexOf(item);
      this.editedItem = await Object.assign({}, item);
      this.openDialog();
    },

    async deleteMeetingType(item) {
      this.editedIndex = this.meetingTypes.indexOf(item);
      ...
      if (delateConfirm) {
        let meetingType = await this.$axios.delete(
          "/api/meetingType/" + this.editedItem.id
        );
    ...
    async save(e) {
      ...
      let meetingType = false;

      if (saveConfirm) {
        this.opensOverlay();

        if (this.editedIndex > -1) {
          meetingType = await this.updateMeetingType();
        } else {
          meetingType = await this.createMeetingType();
        }
    ...
    async updateMeetingType() {
      let updateMeetingType = await this.$axios
        .put("/api/meetingType/" + this.editedItem.id, {
          data: this.editedItem,
        })
        .then((res) => {
          // console.log("res", res.data);
