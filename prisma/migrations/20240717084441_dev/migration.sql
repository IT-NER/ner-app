...
CREATE TABLE [dbo].[Publish] (
    ...
    [start] DATETIME2,
    [end] DATETIME2,
    [timed] BIT CONSTRAINT [Publish_timed_df] DEFAULT 1,
    [remark] NVARCHAR(1000),
    [active] BIT CONSTRAINT [Publish_active_df] DEFAULT 1,
    [contentId] INT,
    [userId] INT,
    [publishStatusId] INT,
    [createdAt] DATETIME2 NOT NULL CONSTRAINT [Publish_createdAt_df] DEFAULT CURRENT_TIMESTAMP,
    [updatedAt] DATETIME2 NOT NULL,
    CONSTRAINT [Publish_pkey] PRIMARY KEY CLUSTERED ([id]),
    CONSTRAINT [Publish_ticket_key] UNIQUE NONCLUSTERED ([ticket])
);
...
CREATE TABLE [dbo].[PublishStatus] (
    [id] INT NOT NULL IDENTITY(1,1),
    [name] NVARCHAR(1000),
    [active] BIT CONSTRAINT [PublishStatus_active_df] DEFAULT 1,
    [createdAt] DATETIME2 NOT NULL CONSTRAINT [PublishStatus_createdAt_df] DEFAULT CURRENT_TIMESTAMP,
    [updatedAt] DATETIME2 NOT NULL,
    CONSTRAINT [PublishStatus_pkey] PRIMARY KEY CLUSTERED ([id])
);
...
CREATE TABLE [dbo].[Content] (
    ...
    [publish] BIT CONSTRAINT [Content_publish_df] DEFAULT 0,
    [contentTypeId] INT,
    [userId] INT,
...
-- AddForeignKey
ALTER TABLE [dbo].[Publish] ADD CONSTRAINT [Publish_contentId_fkey] FOREIGN KEY ([contentId]) REFERENCES [dbo].[Content]([id]) ON DELETE SET NULL ON UPDATE CASCADE;

-- AddForeignKey
ALTER TABLE [dbo].[Publish] ADD CONSTRAINT [Publish_userId_fkey] FOREIGN KEY ([userId]) REFERENCES [dbo].[User]([id]) ON DELETE NO ACTION ON UPDATE NO ACTION;

-- AddForeignKey
ALTER TABLE [dbo].[Publish] ADD CONSTRAINT [Publish_publishStatusId_fkey] FOREIGN KEY ([publishStatusId]) REFERENCES [dbo].[PublishStatus]([id]) ON DELETE SET NULL ON UPDATE CASCADE;
...
-- AddForeignKey
ALTER TABLE [dbo].[PointReceived] ADD CONSTRAINT [PointReceived_publishId_fkey] FOREIGN KEY ([publishId]) REFERENCES [dbo].[Publish]([id]) ON DELETE SET NULL ON UPDATE CASCADE;
