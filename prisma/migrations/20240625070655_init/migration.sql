...
CREATE TABLE [dbo].[Activity] (
    ...
    [description] NVARCHAR(1000),
    [show] BIT,
    [active] BIT CONSTRAINT [Activity_active_df] DEFAULT 1,
    ...
    CONSTRAINT [Activity_code_key] UNIQUE NONCLUSTERED ([code])
);
...
CREATE TABLE [dbo].[ActivityImg] (
    ...
    [path] NVARCHAR(1000),
    [name] NVARCHAR(1000),
    CONSTRAINT [ActivityImg_pkey] PRIMARY KEY CLUSTERED ([id])
);
...
-- AddForeignKey
ALTER TABLE [dbo].[ActivityImg] ADD CONSTRAINT [ActivityImg_activityId_fkey] FOREIGN KEY ([activityId]) REFERENCES [dbo].[Activity]([id]) ON DELETE SET NULL ON UPDATE CASCADE;
